...
#define GATTS_ADV_NAME              "ESP_GATTS"
...
static const char remote_device_name[] = "ESP_GATTS";
...
void redtask(void *pvParameter)
{
    esp_task_wdt_reset(); // add on May 5 2021 @ 17:01
  ...
  while(1)
  {
    ...
    vTaskDelay(5000 / portTICK_PERIOD_MS);                 //changed from 5 to 1.
    ESP_LOGE(COEX_TAG, "PRINTING RED COLOUR\n");
  }
}

void BLUETASK(void *pvParameter)
{
   
  while(1)
  {
    vTaskDelay(5000 / portTICK_PERIOD_MS);                 //changed from 5 to 1.
    ESP_LOGE(COEX_TAG, "PRINTING blue COLOUR\n");
  }
}

void GREENTASK(void *pvParameter)
{
    
  while(1)
  {
    vTaskDelay(5000 / portTICK_PERIOD_MS);                 //changed from 5 to 1.
    ESP_LOGE(COEX_TAG, "PRINTING green COLOUR\n");
  }
}
